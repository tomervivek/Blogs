...
const navigation = [
  ...
  { name: 'Add a blog', href: '/add-new-blog' },
]
...
export default function HeaderSection(props) {
  ...
  return (
    <div className="bg-white">
      <header className=" bg-[#4F6F52] opacity-100 top-0 z-50">
        <nav className="flex items-center justify-between mx-auto max-w-screen-xl p-5" aria-label="Global">
...
<div className='bg-red-500 text-white rounded-full px-1 -ml-2 -mb-1 text-xs'>{checked.length}</div>
          ...
          <div className="hidden lg:flex lg:gap-x-12">
            {navigation.map((item) => (
              <a key={item.name} href={item.href} className="text-sm font-semibold leading-6 hover:text-gray-200 text-white">
                {item.name}
              </a>
            ))}
          </div>
